...
for pth in paths:
    ...
    if sender1 not in disps.keys():
        disps[sender1] = [teta1]
    else:
        disps[sender1].append(teta1)

    if sender2 not in disps.keys():
        disps[sender2] = [teta2]
    else:
        disps[sender2].append(teta2)

for person in disps.keys():
    maximum = max(disps[person])
    minimum = min(disps[person])

    disp = maximum - minimum
    suma = sum(disps[person])
    avg = suma/len(disps[person])

    disps[person] = [disp, avg]
    print(person+": "+str(disp)+" "+str(avg))
...
chat = chatClass("OljaBelic3.html")
chat_len = len(chat.msgs)
